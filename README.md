...
- **Import your course outline** — point at the outline your instructor handed out and it fills in the learning outcomes, what each piece of work is worth, the final exam date and the weekly schedule. Every row is shown with a tick box first; nothing is saved until you confirm it.
- **Marks and "what do I need on the final"** — enter each mark as it comes back and see your standing, how much of the course you have secured, and exactly what the work still to come has to average for you to finish on the number you are aiming at. If the number is out of reach it says so.
- **Exam mode** — within three weeks of an exam, one tap fits the deck to the date: nothing is scheduled past the exam, everything is seen at least twice, and the cards you keep missing come first. Switch it off and the ordinary schedule is exactly as it was.
- **Fairer quizzes** — wrong options come from the same part of the material, are matched in length, and are drawn from the material's own lists where it has one, so an enzyme is answered against the other enzymes rather than against any old word. An option that would make the sentence true as well is never offered — you can't be marked wrong for being right — while a genuine near-miss still is. A question that can't be built fairly isn't asked. And the app never asks a true/false question it could mark wrong: sentences that hedge, negate or contrast are handled carefully rather than swapped blindly.
- **Cleaner material** — the running header, copyright line, "Learning Objectives" and the whole reference list are stripped out of a chapter PDF before anything is built from it, so your cards come from the chapter rather than its furniture. Where the slides and the textbook say the same thing, you are asked about it once.
- **Formulas that read properly** — H₂O, C₆H₁₂O₆, r² and Ca²⁺ keep their subscripts and superscripts out of a PDF instead of being flattened to H2O and r2. Type `H2O` in a quiz and you are still marked right.
- **Figures keep their units** — questions ask for `37 °C` or `2 ATP`, not a bare number, and every option carries the same unit.
- **Better blanks** — a fill-the-gap card is never given away by the answer's own abbreviation sitting in the same sentence, and never left in a sentence starting "This" or "It" where the subject is somewhere else.
