...
- **Fairer quizzes** — wrong options now come from the same part of the material and are matched in length, so you can't pick the answer by its shape. A question that can't be built fairly isn't asked. And the app never asks a true/false question it could mark wrong: sentences that hedge, negate or contrast are handled carefully rather than swapped blindly.
